...
export default function EduPyeAIChat({ customContext, className = '' }: EduPyeAIChatProps) {
  ...
  // In-chat interactive Question Set answers state: { [messageId]: { [questionId]: selectedOptionId } }
  const [quizAnswers, setQuizAnswers] = useState<Record<string, Record<string | number, string>>>({});
              ...
              return (
                <div key={msg.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                  ...
                  >
                    ...
                    {/* Interactive Question Set Card (when AI generates a multi-question practice quiz) */}
                    {msg.questionSet && msg.questionSet.questions && msg.questionSet.questions.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-200/80">
                        {/* Quiz Header Badges & Live Score */}
                        <div className="flex flex-wrap items-center justify-between gap-1.5 mb-2.5">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="px-2 py-0.5 rounded-md bg-[#0091ff]/15 text-[#0070cc] font-extrabold text-[10px]">
                              {msg.questionSet.board} • {msg.questionSet.subject}
                            </span>
                            <span className="px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 font-bold text-[10px] border border-purple-200">
                              Quiz: {msg.questionSet.topic}
                            </span>
                          </div>

                          {/* Live Score Counter */}
                          {(() => {
                            const answers = quizAnswers[msg.id] || {};
                            let correctCount = 0;
                            msg.questionSet.questions.forEach((q) => {
                              if (answers[q.id] === q.correctAnswer) correctCount++;
                            });
                            const answeredCount = Object.keys(answers).length;
                            return (
                              <span className="text-[10px] font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full">
                                Score: {correctCount}/{msg.questionSet.questions.length} ({answeredCount} answered)
                              </span>
                            );
                          })()}
                        </div>

                        {/* Direct Clickable Link to full test mode */}
                        <div className="my-2 p-2 bg-gradient-to-r from-blue-50 to-indigo-50/70 rounded-xl border border-blue-200/90 shadow-2xs">
                          <button
                            onClick={() => handleLaunchTest(msg.questionSet!.testId)}
                            className="text-[#0066cc] hover:text-[#004c99] font-extrabold text-xs flex items-center justify-between w-full group/link cursor-pointer text-left"
                          >
                            <span className="flex items-center gap-1.5 underline decoration-2 underline-offset-2">
                              <span>👉 Solve all {msg.questionSet.questions.length} questions in untimed test mode</span>
                            </span>
                            <ExternalLink className="w-3.5 h-3.5 flex-shrink-0 group-hover/link:translate-x-0.5 transition-transform" />
                          </button>
                        </div>

                        {/* Stepper Tabs & Question View */}
                        {(() => {
                          const currentIdx = activeQuizIndices[msg.id] || 0;
                          const currentQ = msg.questionSet.questions[currentIdx] || msg.questionSet.questions[0];
                          const currentAnswers = quizAnswers[msg.id] || {};
                          const selectedOpt = currentAnswers[currentQ.id];
                          const hasAnsweredCurrent = Boolean(selectedOpt);

                          return (
                            <div className="space-y-2.5">
                              {/* Question number selector tabs */}
                              <div className="flex items-center gap-1.5 overflow-x-auto pb-1">
                                {msg.questionSet.questions.map((q, qIdx) => {
                                  const ans = currentAnswers[q.id];
                                  const isAns = Boolean(ans);
                                  const isCorr = ans === q.correctAnswer;
                                  const isActive = qIdx === currentIdx;

                                  let tabClass = 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200';
                                  if (isAns) {
                                    tabClass = isCorr
                                      ? 'bg-emerald-100 text-emerald-800 border-emerald-300 font-bold'
                                      : 'bg-rose-100 text-rose-800 border-rose-300 font-bold';
                                  } else if (isActive) {
                                    tabClass = 'bg-[#0091ff] text-white border-[#0091ff] font-bold shadow-xs';
                                  }
                                  ...
                                  return (
                                    <button
                                      key={q.id}
                                      onClick={() =>
                                        setActiveQuizIndices((prev) => ({ ...prev, [msg.id]: qIdx }))
                                      }
                                      className={`px-2.5 py-1 rounded-lg border text-[10px] flex items-center gap-1 transition-all cursor-pointer ${tabClass}`}
                                    >
                                      <span>Q{qIdx + 1}</span>
                                      {isAns && (isCorr ? '✓' : '✗')}
                                    </button>
                              ...
                              <div className="p-3 bg-white border border-slate-200 rounded-xl shadow-2xs space-y-2">
                                <div className="flex items-center justify-between text-[10px] text-slate-400 font-bold">
                                  <span>Question {currentIdx + 1} of {msg.questionSet.questions.length}</span>
                                  {currentQ.difficulty && (
                                    <span className="px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200 text-[9px] font-extrabold">
                                      {currentQ.difficulty}
                                    </span>
                                  )}
                                </div>

                                <p className="font-bold text-[#1c3352] text-xs leading-snug">
                                  {currentQ.question}
                                </p>
                                  ...
                                  {currentQ.options.map((opt) => {
                                    ...
                                    if (hasAnsweredCurrent) {
                                      if (isSelected && isCorrect) {
                                        ...
                                        btnStyle = 'bg-emerald-50/60 border-emerald-300 text-emerald-700 font-semibold';
                                      }
                                    }
                                ...
                                {hasAnsweredCurrent && (
                                  <div className="p-2.5 rounded-xl bg-blue-50/70 border border-blue-100 text-[11px] text-slate-700 animate-in fade-in duration-150">
                                    <p className="font-bold text-[#0070cc] mb-1">Pedagogical Explanation:</p>
                                ...
                                <div className="flex items-center justify-between pt-1">
                                  <button
                                    ...
                                    onClick={() =>
                                      setActiveQuizIndices((prev) => ({ ...prev, [msg.id]: currentIdx - 1 }))
                                    }
                                    className="px-2.5 py-1 text-[10px] font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 disabled:opacity-30 rounded-lg cursor-pointer transition-colors"
                                  >
                                    ← Previous
                                  </button>

                                  <button
                                    disabled={currentIdx === msg.questionSet.questions.length - 1}
                                    onClick={() =>
                                      setActiveQuizIndices((prev) => ({ ...prev, [msg.id]: currentIdx + 1 }))
                                    }
                                    className="px-3 py-1 text-[10px] font-bold text-[#0070cc] bg-blue-50 hover:bg-blue-100 disabled:opacity-30 rounded-lg cursor-pointer transition-colors"
                                  >
                                    Next Question →
                                  </button>
                                </div>
                        ...
                        {/* Direct solve in test mode button */}
                        <div className="pt-2">
                          <button
                            onClick={() => handleLaunchTest(msg.questionSet!.testId)}
                            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-[#0091ff] to-[#0066cc] text-white font-black text-xs hover:from-[#0080e6] hover:to-[#0055b3] shadow-md hover:shadow-lg transition-all active:scale-98 cursor-pointer"
                          >
                            <span>🚀 Solve in Test Mode (Untimed)</span>
                            <ExternalLink className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      </div>
